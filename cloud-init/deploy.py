...
--
python3 friendly and python2 compliant
'''
...
class Launch(object):
    ...
    def __init__(self, userdata, hostname='missing', sshkey=None):
        '''
        start instance
...
def main():
    ...
    try:
        parser = argparse.ArgumentParser(
                description='deployment tool for AWS',
                epilog='ssh keypair is name in AWS and ~/.ssh/')
        parser.add_argument('-t', '--templatefile',
                            help='Jinja2 userdata template file')
        parser.add_argument('--sshkey',
                            help='AWS keypair name for SSH, ~/.ssh/{name}',
                            default='initial')
    ...
    my_inst = Launch(my_data, args.hostname, args.sshkey)
    my_inst.addsecurity()
    my_inst.launch()
